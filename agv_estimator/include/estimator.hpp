...
public:
    ...
    // 예측 단계 : 사용자의 입력(command)을 가했을 때 예상되는 측정값(statePre) 계산
    ...
    cv::Mat_<float> correct()
    {
        ...
        if (received)
            sensor_marker = posData.clone();
        else
            kf.statePost.rowRange(0, 3).copyTo(sensor_marker);
    ...
    cv::Mat_<float> EncorderCorrect()
    {   
        cv::Mat none = kf.statePost.rowRange(0, 3).clone();
        cv::Mat sensor_encoder = velData.clone();
        cv::Mat encoder;
        cv::vconcat(none, sensor_encoder, encoder);
        kf.correct(encoder);
        return kf.statePost;
    }
